...
import { ChevronLeft, ChevronRight, Plus, Clock, User, Edit, Trash2, ListTodo, CalendarPlus } from "lucide-react";
...
// --- Types ---
interface Session {
  id: string; title: string; session_date: string; duration_minutes: number;
  status: string; client_id: string; description: string | null; client_name?: string;
}
interface StaffTodo {
  id: string; title: string; due_date: string | null; is_completed: boolean;
  assigned_to: string; created_by: string; assigned_name?: string; description: string;
}
interface ClientProfile { id: string; full_name: string; }
...
const StatusBadge = ({ status }: { status: string }) => {
  const colors: Record<string, string> = {
    ...
    "no-show": "bg-yellow-500/15 text-yellow-700",
  };
  return <span className={`text-[10px] px-2 py-0.5 rounded-full font-medium ${colors[status] || "bg-primary/15 text-primary"}`}>{status}</span>;
};

const DayCell = ({ day, sessions, todos, selectedDate, onSelect }: {
  day: Date; sessions: Session[]; todos: StaffTodo[];
  ...
  >
    ...
    <div className="mt-1 space-y-0.5">
      {sessions.slice(0, 2).map((s) => (
        <div key={s.id} className={`text-[10px] rounded px-1.5 py-0.5 truncate font-medium ${s.status === "completed" ? "bg-green-500/15 text-green-700" : s.status === "cancelled" ? "bg-red-500/15 text-red-700" : "bg-primary/15 text-primary"}`}>
      ...
      {(sessions.length + todos.length) > 4 && (
        <div className="text-[10px] text-muted-foreground">+{sessions.length + todos.length - 4} more</div>
      )}
    </div>
  </div>
);
...
const AdminCalendar = () => {
  ...
  const [newSession, setNewSession] = useState({ title: "", client_id: "", session_date: "", session_time: "09:00", duration_minutes: 60, description: "" });
  ...
  const [newTask, setNewTask] = useState({ title: "", assigned_to: "", due_date: "", description: "" });

  // Create dialog tab (session or task)
  const [createTab, setCreateTab] = useState<"session" | "task">("session");
  ...
  const fetchSessions = async () => {
    ...
    if (data) {
      ...
      if (clientIds.length > 0) {
        ...
        setSessions(data.map((s) => ({ ...s, client_name: "Unknown" })));
      }
    }
  };
  ...
  const fetchTodos = async () => {
    ...
    if (data) {
      ...
      if (userIds.length > 0) {
        ...
        setTodos(data.map((t) => ({ ...t, assigned_name: "Unknown" })));
      }
    }
  };
  ...
  const handleCreateSession = async () => {
    ...
    else { toast.success("Session created"); setCreateOpen(false); setNewSession({ title: "", client_id: "", session_date: "", session_time: "09:00", duration_minutes: 60, description: "" }); fetchSessions(); }
  };

  const openEdit = (s: Session) => {
    const d = new Date(s.session_date);
    setEditSession(s);
    setEditForm({ title: s.title, session_date: format(d, "yyyy-MM-dd"), session_time: format(d, "HH:mm"), duration_minutes: s.duration_minutes, description: s.description || "", status: s.status });
    setEditOpen(true);
  };
  ...
  const handleCreateTask = async () => {
    if (!newTask.title || !newTask.assigned_to || !user) return;
    const { error } = await supabase.from("staff_todos").insert({
      title: newTask.title, assigned_to: newTask.assigned_to, created_by: user.id,
      due_date: newTask.due_date || null, description: newTask.description || "",
    });
    if (error) toast.error("Failed to create task");
    else { toast.success("Task created"); setCreateOpen(false); setNewTask({ title: "", assigned_to: "", due_date: "", description: "" }); fetchTodos(); }
  };

  // Open create dialog pre-filled with selected date
  const openCreateForDate = (date: Date) => {
    const dateStr = format(date, "yyyy-MM-dd");
    setNewSession((prev) => ({ ...prev, session_date: dateStr }));
    setNewTask((prev) => ({ ...prev, due_date: dateStr }));
    setCreateTab("session");
    setCreateOpen(true);
  };
      ...
      <section className="pt-28 pb-20">
        <div className="container max-w-6xl">
          <div className="flex items-center justify-between mb-8">
            ...
            <Button className="rounded-full gap-2" onClick={() => { setCreateTab("session"); setCreateOpen(true); }}>
              <Plus size={16} /> New
            </Button>
          </div>
          ...
          <div className="bg-card border border-border/50 rounded-2xl overflow-hidden">
            ...
            <div className="grid grid-cols-7">
              ...
              {days.map((day) => (
                <DayCell key={day.toISOString()} day={day} sessions={sessionsForDay(day)} todos={todosForDay(day)} selectedDate={selectedDate} onSelect={handleDateSelect} />
              ))}
            </div>
          </div>
          ...
          {selectedDate && (
            <div className="mt-6 bg-card border border-border/50 rounded-2xl p-6">
              <div className="flex items-center justify-between mb-4">
                ...
                <Button size="sm" variant="outline" className="rounded-full gap-2" onClick={() => openCreateForDate(selectedDate)}>
                  <CalendarPlus size={14} /> Add to this day
                </Button>
              </div>
                  ...
                  {sessionsForDay(selectedDate).map((s) => (
                    <div key={s.id} className="flex items-center justify-between bg-background rounded-xl p-4 border border-border/30">
                      <div className="flex items-center gap-4">
                        <div className="flex flex-col items-center rounded-lg px-3 py-2 bg-primary/10">
                          <Clock size={14} className="text-primary" />
                          <span className="text-xs font-semibold mt-1 text-primary">{format(new Date(s.session_date), "HH:mm")}</span>
                        </div>
                        <div>
                      ...
                      <div className="flex items-center gap-2">
                        ...
                        <Button size="icon" variant="ghost" className="h-8 w-8" onClick={() => openEdit(s)}><Edit size={14} /></Button>
                        <Button size="icon" variant="ghost" className="h-8 w-8 text-destructive" onClick={() => handleDelete(s.id)}><Trash2 size={14} /></Button>
                      </div>
                    </div>
                  ))}
                </div>
          ...
          {/* Create Session/Task Dialog */}
          <Dialog open={createOpen} onOpenChange={setCreateOpen}>
            <DialogContent className="sm:max-w-lg">
              <DialogHeader><DialogTitle>Create New</DialogTitle></DialogHeader>
              <Tabs value={createTab} onValueChange={(v) => setCreateTab(v as "session" | "task")} className="mt-2">
                <TabsList className="w-full">
                  <TabsTrigger value="session" className="flex-1 gap-1"><Clock size={14} /> Session</TabsTrigger>
                  <TabsTrigger value="task" className="flex-1 gap-1"><ListTodo size={14} /> Task</TabsTrigger>
                </TabsList>
                <TabsContent value="session" className="space-y-4 mt-4">
                  ...
                  <div className="grid grid-cols-2 gap-3">
                    <div><Label className="text-xs">Date</Label><Input type="date" value={newSession.session_date} onChange={(e) => setNewSession({ ...newSession, session_date: e.target.value })} /></div>
                    <div><Label className="text-xs">Time</Label><Input type="time" value={newSession.session_time} onChange={(e) => setNewSession({ ...newSession, session_time: e.target.value })} /></div>
                  </div>
                  ...
                  <Button onClick={handleCreateSession} className="w-full rounded-full">Create Session</Button>
                </TabsContent>
                <TabsContent value="task" className="space-y-4 mt-4">
                  <Input placeholder="Task title" value={newTask.title} onChange={(e) => setNewTask({ ...newTask, title: e.target.value })} />
                  <Select value={newTask.assigned_to} onValueChange={(v) => setNewTask({ ...newTask, assigned_to: v })}>
                    <SelectTrigger><SelectValue placeholder="Assign to staff member" /></SelectTrigger>
                  ...
                  <Button onClick={handleCreateTask} className="w-full rounded-full">Create Task</Button>
                </TabsContent>
              </Tabs>
            </DialogContent>
          </Dialog>
          ...
          <Dialog open={editOpen} onOpenChange={setEditOpen}>
            <DialogContent>
              ...
              <div className="space-y-4 mt-4">
                <div><Label className="text-xs">Title</Label><Input value={editForm.title} onChange={(e) => setEditForm({ ...editForm, title: e.target.value })} /></div>
                <div className="grid grid-cols-2 gap-3">
                  <div><Label className="text-xs">Date</Label><Input type="date" value={editForm.session_date} onChange={(e) => setEditForm({ ...editForm, session_date: e.target.value })} /></div>
                  <div><Label className="text-xs">Time</Label><Input type="time" value={editForm.session_time} onChange={(e) => setEditForm({ ...editForm, session_time: e.target.value })} /></div>
                </div>
                <div><Label className="text-xs">Duration</Label>
                  <Select value={String(editForm.duration_minutes)} onValueChange={(v) => setEditForm({ ...editForm, duration_minutes: Number(v) })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent><SelectItem value="30">30 min</SelectItem><SelectItem value="45">45 min</SelectItem><SelectItem value="60">60 min</SelectItem><SelectItem value="90">90 min</SelectItem><SelectItem value="120">120 min</SelectItem></SelectContent>
                  </Select>
                </div>
                <div><Label className="text-xs">Status</Label>
                  <Select value={editForm.status} onValueChange={(v) => setEditForm({ ...editForm, status: v })}>
                    <SelectTrigger><SelectValue /></SelectTrigger>
                    <SelectContent><SelectItem value="scheduled">Scheduled</SelectItem><SelectItem value="completed">Completed</SelectItem><SelectItem value="cancelled">Cancelled</SelectItem><SelectItem value="no-show">No Show</SelectItem></SelectContent>
                  </Select>
                </div>
                <div><Label className="text-xs">Description</Label><Textarea value={editForm.description} onChange={(e) => setEditForm({ ...editForm, description: e.target.value })} /></div>
              </div>
              <DialogFooter>
